...
The Vector distribution keeps a compact, license-preserving icon set below the
host's plugin file limit:

| Library | Style | Count |
|---------|-------|-------|
| `chunk-filled` | fill / compact, chunky 16px silhouettes | complete library |
| `tabler-filled` | fill / bezier-curve forms | complete library |
| `phosphor-duotone` | duotone / single color + 0.2 opacity backplate | complete library |
| `simple-icons` | brand logos (company / product marks) | curated common subset |
...
[`sounds/`](./sounds/) retains upstream planning metadata and attribution, but
the Vector package does not bundle WAV binaries. Use user-supplied or otherwise
properly licensed project-local audio only after a concrete auditory job exists.
See [usage](./sounds/README.md).
