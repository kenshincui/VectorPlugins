...
If no concrete cue is selected, do not create `<project_path>/sounds/`. If a
cue is selected, use user-supplied or independently sourced, properly licensed
project-local audio, reference its relative path from the sidecar, record its
provenance, and validate the sidecar before export. A
...
`templates/sounds/` directly and the Vector package contains no bundled WAVs.
