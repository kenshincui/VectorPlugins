...
| An audio cue fits one resolved transition or animation row | Use user-supplied or independently sourced, properly licensed project-local audio and record its provenance |
...
Use the corresponding project-relative `sounds/<file>` path in
...
Vector package contains no bundled sound library; silence remains valid.
