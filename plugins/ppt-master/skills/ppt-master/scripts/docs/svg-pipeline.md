...
- Transition/object sound remains off by default. After SVG and visual motion are complete and one row has a concrete auditory job, use user-supplied or independently sourced, properly licensed project-local audio and record its provenance. `transition.sound` references a project-relative `.wav`; object-animation `sound` accepts the existing `.m4a`/`.mp3`/`.wav` path contract. With no selected cue, do not create `<project>/sounds/`. Export never resolves ids or reads `templates/sounds/` directly; the Vector package contains no bundled WAVs
...
Replaces `<use data-icon="chunk-filled/name" .../>`, `<use data-icon="tabler-filled/name" .../>`, and `<use data-icon="phosphor-duotone/name" .../>` placeholders with actual SVG path elements. Existing project-local icon namespaces remain compatible. Use for manual icon embedding checks outside `finalize_svg.py`.
