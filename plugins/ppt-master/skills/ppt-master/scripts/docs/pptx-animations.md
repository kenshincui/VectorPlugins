...
Sound selection is a workflow concern, not part of the animation core. The
Vector distribution omits bundled WAV binaries: after the SVG and object-motion
solution are complete, use properly licensed project-local `.m4a`, `.mp3`, or
`.wav` inputs and store their relative paths in new sidecars. The core never
resolves a library id or reads `templates/sounds/` directly; see
[`animations.md`](../../references/animations.md) §2.2.
