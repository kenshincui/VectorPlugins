...
>
> **stroke-width** is legacy compatibility for existing project-local stroke libraries only. Allowed values are `{1.5, 2, 3}`; when an existing lock declares it, all matching placeholders MUST use that value deck-wide. New Vector bundled-library selections do not declare it.
>
