...
>
> 1. **Pick at most one primary stylistic library from the three bundled choices** — when generic icons are needed, read the source material and choose the one whose visual character best serves the deck:
...
>    - During bundled-library selection, do not select generic icons from more than one of `chunk-filled` / `tabler-filled` / `phosphor-duotone`. If the chosen library lacks an exact icon, find the closest alternative **within that same library**.
...
> 2. **Legacy stroke compatibility** — retain a deck-wide `stroke_width` only when an existing project already supplies a project-local stroke library. New Vector bundled-library selections do not introduce a stroke lock.
>
...
> 6. Keep each successful, case-sensitive `lib/name`: bundled basenames are lowercase (`tabler-filled/award`, never `tabler-filled/Award`); custom icons retain exact case.
