...
| Source | Action |
|---|---|
| Upstream sound vocabulary | Planning reference only in the Vector distribution; obtain a properly licensed file independently, copy it into the project, and record its provenance |
...
```bash
# Verify the independently supplied project-local audio before use
test -f <project_path>/sounds/<file>
```

The Vector package omits bundled WAV binaries. The exporter never reads
`templates/sounds/` directly; sidecars store project-relative paths rather than
library ids. Silence is valid and preferred when no licensed local cue exists.
