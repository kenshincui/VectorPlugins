...
The research notes are the ONLY communication channel between optional workers
and the coordinator. Every fact in the final report must be traceable to a line in
...
workspace/research-notes/
  task-a.md       Worker A or sequential specialist writes
  task-b.md       Worker B or sequential specialist writes
  task-c.md       Worker C or sequential specialist writes
  task-d.md       Worker D or sequential specialist writes
  registry.md     Coordinator builds from task-*.md (P3)
```
...
- The coordinator will reassign GLOBAL [n] numbers in registry.md
- URL must be from an actually retrieved result (workers MUST NOT invent URLs)
...
For each source whose full content was read:
...
List what the worker searched for but could NOT find, and possible counter-readings.
