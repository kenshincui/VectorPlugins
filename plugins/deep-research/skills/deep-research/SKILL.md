...
## Architecture: Coordinator + Optional Parallel Workers

```
Coordinator (minimizes raw retrieval context)
  |
  P0: Environment + source policy setup
  |
  P1: Research Task Board (roles, queries, parallel groups)
  |
  Dispatch ──→ Worker A ──→ writes task-a.md ──┐
           ──→ Worker B ──→ writes task-b.md ──┤ (parallel when available)
           ──→ Worker C ──→ writes task-c.md ──┘
  |                                               |
  |     research-notes/  <────────────────────────┘
  |
...
**Context efficiency:** When the host supports parallel workers, their raw retrieval results stay in their own context and only distilled notes return to the coordinator. Otherwise execute the same task board sequentially and write one notes file per task.
...
|-------|-------------|-------------------|
| Search or browser retrieval available | Required | Stop and explain the missing capability |
| Full-page retrieval available | Required for DEEP tasks | SCAN-only mode |
| Parallel worker dispatch | Preferred | Execute tasks sequentially |
...
Report: `[P0 complete] Parallel workers: {yes/no}. Mode: {standard/lightweight}. AS_OF: {YYYY-MM-DD}.`
...
Workers execute tasks using [references/subagent_prompt.md](references/subagent_prompt.md) and output to [references/research_notes_format.md](references/research_notes_format.md).

### With Parallel Workers

1. Dispatch Group A tasks in parallel (max 3 concurrent)
2. Each worker uses the retrieval capabilities available in the host and tags source types
...
The coordinator executes tasks sequentially, acting as each specialist. Raw retrieval results are discarded after writing notes.
...
The coordinator reads all task notes and builds a unified registry.
...
The coordinator reads notes + registry to build the outline.
...
**Anti-hallucination:**
- The coordinator never invents URLs — use only URLs actually retrieved and recorded in task notes
- The coordinator never fabricates data — mark [unverified] if a number is not in the notes
...
### Using Parallel Counter-Review Workers (When Available)
...
# 2. Dispatch four independent review tasks through the host's worker mechanism
claim-validator
...
# 4. Return the four reports to the coordinator for synthesis
...
| [subagent_prompt.md](references/subagent_prompt.md) | P2: Task dispatch to optional workers |
| [research_notes_format.md](references/research_notes_format.md) | P2: Worker output format |
...
- **Coordinator repeatedly rereading raw retrieval results** — write and use distilled task notes
...
Options:
A) Verify facts and citations (Recommended)
B) Create slides from the findings using an installed presentation Skill
C) Export a formal document or PDF using an installed document Skill
